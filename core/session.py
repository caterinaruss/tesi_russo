...
class Session:
    def __init__(
        ...
        verbose: bool = False,
        table_detection_results: Optional[dict] = None,
    ):
        ...
        self.table_detection_results = table_detection_results
        ...
        if self.with_retriever:
            ...
            self.vector_store = MilvusStore(**config_milvus, model_type=self.model_type)
    ...
    def construct_planner_prompt(self, key_info: Optional[str]):
        ...
        table_info_str = ""
        if self.table_detection_results:
            table_info_parts = ["\nHere is some information about the tables detected in the workbook, which might be helpful:"]
            for sheet_res in self.table_detection_results.get('sheets', []):
                if sheet_res['num_tables'] > 0:
                    table_info_parts.append(f"  - In sheet '{sheet_res['sheet_name']}': {sheet_res['num_tables']} table(s) found.")
                    for table in sheet_res['tables']:
                        bbox = table['bbox']
                        table_info_parts.append(f"    - Table {table['table_id']} is located at bounding box (row_start, col_start, row_end, col_end): {bbox}.")
            table_info_str = "\n".join(table_info_parts) + "\n"
        
        context += table_info_str
